...
class AnagramsSuite extends FunSuite {
  ...
  test("sentenceOccurrences: []") {
    assert(sentenceOccurrences(List[Word]()) === List[(Char, Int)]())
  }
  ...
  test("combinations: aa") {
    assert(combinations(List(('a', 2))) === List(List(), List(('a', 1)), List(('a', 2))))
  }

  test("combinations: ai") {
    assert(combinations(List(('a', 1), ('i', 1))) ===
      List(List(), List(('a', 1)), List(('i', 1)), List(('a', 1), ('i', 1))))
  }
